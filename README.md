# webhook-irc-relay

Webhook relay for Swift/Vapor that normalizes incoming events and forwards them to IRC channels.

## What it does
- Receives `POST /webhooks/:source` requests.
- Normalizes payloads into a common domain event shape.
- Routes each source to a specific IRC channel.
- Joins configured IRC channels and announces presence with a startup message.
- Sends formatted IRC `PRIVMSG` lines over a persistent TCP connection.
- Publishes API docs with OpenAPI at `/openapi.json` and Swagger UI at `/swagger`.
- Optionally forwards warning/error logs to Sentry in release mode when `SENTRY_DSN` is configured.
- Emits structured logs across HTTP ingress, routing, normalization, and IRC transport with request correlation metadata.

## Observability
- Every request receives or propagates an `X-Request-Id` header.
- Logs include request and troubleshooting context such as source, event type, payload size, user agent, remote address, IRC channel, and transport state.
- Structured logs are emitted as JSONSeq through `JSONLogger`.
- Warning and error logs can be forwarded to Sentry through `SwiftSentry` while JSON logging remains enabled.
- Warning and error events sent to Sentry are enriched with a short in-memory breadcrumb trail built from recent application logs, scoped by `request_id` when available.
- IRC connection lifecycle events are logged, including connect, handshake, JOIN, disconnect, reconnect, queueing, and message flush.

## Swagger/OpenAPI policy
- Disabled by default in production.
- Enabled by default outside production.
- Can be overridden with `SWAGGER_ENABLED`.

## Supported sources
- bazarr
- radarr
- sonarr
- lidarr
- prowlarr
- fail2ban

## Quick start
1. Export the environment variables described in [docs/SETUP.md](docs/SETUP.md).
2. Build.
```bash
swift build
```
3. Run.
```bash
swift run
```
4. Test.
```bash
swift test
```

## Docker
Build the image:
```bash
docker build -f docker/Dockerfile -t webhooks-irc:latest .
```

Run the image:
```bash
docker run --rm -p 8080:8080 \
    -e IRC_HOST=irc.example.net \
    -e IRC_PORT=6667 \
    -e IRC_NICK=webhooks-bot \
    webhooks-irc:latest
```

The Docker build caches SwiftPM resolution/build artifacts and only copies the Swift sources required for compilation.

## Project structure

```
src/
├── Code/
│   ├── Adapters/Network/Handlers/   # Webhook payload decoders (Radarr, Sonarr, ...)
│   ├── Application/
│   │   ├── App/Middleware/          # Vapor bootstrap, routes, middleware, DTOs
│   │   ├── Services/                # HandlerRegistry, IRCChannelRouter, WebhookDispatcher
│   │   └── UseCases/                # ProcessWebhookUseCase
│   ├── Core/
│   │   ├── Models/                  # IRCChannel, IRCMessage, WebhookEvent, ...
│   │   ├── Errors/                  # IRCError, WebhookError
│   │   └── Protocols/               # WebhookHandlerProtocol, IRCClientProtocol
│   └── Shared/Extensions/           # JSONDecoder+, String+IRCSanitize, ContextualSentryLogHandler
└── Tests/
    ├── App/                         # HTTP integration tests (XCTVapor)
    ├── Application/                 # Services/UseCases unit tests
    ├── Adapters/Handlers/           # Handler unit tests
    ├── Shared/                      # Shared utilities tests
    └── Helpers/                     # MockIRCClient, TestWebhookHandler
```

## Documentation
- [docs/APP.md](docs/APP.md)
- [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)
- [docs/FEATURES.md](docs/FEATURES.md)
- [docs/SETUP.md](docs/SETUP.md)
- [docs/STACK.md](docs/STACK.md)
- [docs/ADR/](docs/ADR/)
